Function Get-CapacidadesVSAN_Excel(){
	$clusters = @()
	$vsans = @()
	$result = @()
	...
	foreach ($cluster in $clusters){
		...
		foreach($ds_vsan in $ds_vsans){
			...
			#}
			$workbook.ActiveSheet.Name = "VSAN-$cluster-$datacenter"
			...
			$escribe.Cells.Item(7,2).Addcomment("Capacidad total de la VSAN (TB)")
			$escribe.Cells.Item(8,2).Addcomment("Espacio Usado en la VSAN (TB)")
			$escribe.Cells.Item(9,2).Addcomment("Espacio Libre VSAN (TB)")
			$escribe.Cells.Item(10,2).Addcomment("Espacio Provisionado por todas la maquinas virtuales")
			$escribe.Cells.Item(7,4).Addcomment("Penalizacion del 30% por manejo interno y alta disponibilidad (HA)")
			$escribe.Cells.Item(8,4).Addcomment("Resultado capacidad total - 30% Penalidad")
			$escribe.Cells.Item(7,6).Addcomment("Provision de disco de todas las maquinas virtuales del cluster")
			$escribe.Cells.Item(8,6).Addcomment("Provision de RAM de todas las maquinas del cluster")
			$escribe.Cells.Item(9,6).Addcomment("Resultado de RAM aplicando politica FTT")
			$escribe.Cells.Item(10,6).Addcomment("Total de provision en discos VSAN")
			$escribe.Cells.Item(13,3).Addcomment("Espacio habilitado para USO DE DISCO")
			$escribe.Cells.Item(13,4).Addcomment("Uso de disco actual en la VSAN")
			$escribe.Cells.Item(13,5).Addcomment("Espacio Disponible para provision")
			$escribe.Cells.Item(16,3).Addcomment("Espacio Habilitado para USO DE DISCO")
			$escribe.Cells.Item(16,4).Addcomment("Provision de disco actual")
			$escribe.Cells.Item(16,5).Addcomment("Disponibilidad de disco para provision")						
