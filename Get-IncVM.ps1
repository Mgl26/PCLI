	<#	
	Detalle: Funcion que evalua los datos actuales de una maquina virtual para facilitar un diagnostico ante un incidente
	Detalle Ejemplo: Get-IncVM [Nobre-VM]
	#>
	...
	function Get-IncVM()
	{
		...
		foreach($vm in $vms){
			...
			Write-host "-------------- Datos Discos y Particiones $vm -------------" -ForegroundColor Yellow
				...
				Write-host ""
				Write-host ""
			Write-host "-------------- Ultimo reinicio de $vm -------------" -ForegroundColor Yellow
				Write-host ""
				Write-host ""					
